...
//const unsigned char VERSION_str[] MEM2_TEXT = "Tester 1.08.001\n";
const unsigned char VERSION_str[] MEM2_TEXT = "Result:\n";
...
//     DisplayLine("wait for key");
     delay(100);
  }
  while(!TestKey) {
     TestKey = digitalRead(TestKeyPin);
     DisplayLine("key pressed");
     delay(100);
  }
  delay(100);
...
void DisplayValue(unsigned long Value, int8_t Exponent, unsigned char Unit, unsigned char digits)
{
  ...
  {
    lcd_big_data(OutBuffer[Index]);   // display char
    Index++;        // next one
    if (Index == Exponent) {
  ...
  if (Prefix != 0) lcd_big_data(Prefix);
  if (Unit) lcd_big_data(Unit);
}
...
//  lcd_command(CMD_SetCGRAMAddress | (location << 3));
  for(uint8_t i=0;i<8;i++) {
   display.write(pgm_read_byte(chardata));
   Serial.write(pgm_read_byte(chardata));
    chardata++;
  }
  display.display();  
}
...
void lcd_big_data(unsigned char temp1) {
  DisplayBigChar(temp1);
}
